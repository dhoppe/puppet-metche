...
define metche::define (
  ...
  $config_file_options_hash = $::metche::config_file_options_hash,
) {
  ...
  $_config_file_path  = pick($config_file_path, "${::metche::config_dir_path}/${name}")
  $_config_file_owner = pick($config_file_owner, $::metche::config_file_owner)
  $_config_file_group = pick($config_file_group, $::metche::config_file_group)
  $_config_file_mode = pick($config_file_mode, $::metche::config_file_mode)
  $config_file_content = default_content($config_file_string, $config_file_template)

  $_config_file_require = pick($config_file_require, $::metche::config_file_require)
